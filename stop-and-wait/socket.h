...
char* receive_from_socket (int socket_fd) {
	...
	size_t length = DEFAULT_BUFFER_SIZE;
	int bytes_recvd;
	while ((bytes_recvd = recv(socket_fd, buffer, length, 0)) != 0) {
		if (bytes_recvd < 0) {
...
int receive_in_buffer (int socket_fd, void* buffer, size_t size) {
	#ifndef SUPPRESS_SOCKET_LOGS
	printf("\nreceiving");
	#endif
	int bytes_recvd;
	while ((bytes_recvd = recv(socket_fd, buffer, size, 0)) != 0) {
		if (bytes_recvd < 0) {
			printf("Error in receiving.");
			exit(1);
		}		
...
void send_on_socket(int socket_fd, char* buffer) {
	check(send(socket_fd, buffer, strlen(buffer), 0) < 0, "Error in sending.");
	#ifndef SUPPRESS_SOCKET_LOGS 
	printf("\nData sent.");
	#endif
}

void send_buffer (int socket_fd, void* buffer, size_t length) {
	check(send(socket_fd, buffer, length, 0) < 0, "Error in sending.");
	#ifndef SUPPRESS_SOCKET_LOGS 
	printf("\nData sent.");
	#endif	
}
